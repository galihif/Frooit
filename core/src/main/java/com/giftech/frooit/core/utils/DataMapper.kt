...
object DataMapper {
    ...
    fun mapResponseToEntity(input:List<FruitResponse>) =
        input.map {
            com.giftech.frooit.core.data.source.local.entity.FruitEntity(
                id = it.id!!,
    ...
    fun mapEntitiesToDomain(input: List<com.giftech.frooit.core.data.source.local.entity.FruitEntity>) =
        input.map {
            Fruit(
                id = it.id,
                name = it.name,
